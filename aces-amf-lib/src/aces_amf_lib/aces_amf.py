...
class ACESAMF:
    ...
    def _ensure_minimal_amf_structure(self):
        ...
        if self.amf.pipeline is None:
            self.amf.pipeline = amf_v2.PipelineType()
            self._has_modifications = True

        if self.amf.pipeline.pipeline_info is None:
            # Use default ACES 1.3.0 if not specified
            default_version = amf_v2.VersionType(major_version=1, minor_version=3, patch_version=0)
            self.amf.pipeline.pipeline_info = amf_v2.PipelineInfoType(
    ...
    def rev_up(self, force: bool = False):
        ...
        self.amf.amf_info.date_time.modification_date_time = amf_utilities.amf_timestamp_string()
        self.amf.amf_info.uuid = uuid4().urn

        self.amf.pipeline.pipeline_info.date_time.modification_date_time = amf_utilities.amf_timestamp_string()

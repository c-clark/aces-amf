...
def _decode_uris_in_object(obj: Any) -> None:
    """
    Recursively decode URI-encoded strings in 'file' attributes of dataclass objects.
    ...
    for field in fields(obj):
        ...
        if field.name == "file":
            if isinstance(value, str):
                setattr(obj, field.name, unquote(value))
            elif isinstance(value, list):
                setattr(obj, field.name, [unquote(v) if isinstance(v, str) else v for v in value])
...
def _encode_uris_in_object(obj: Any) -> None:
    """
    Recursively encode strings in 'file' attributes of dataclass objects.
    ...
    for field in fields(obj):
        ...
        if field.name == "file":
            if isinstance(value, str):
                setattr(obj, field.name, quote(value, safe=""))
            elif isinstance(value, list):
                setattr(obj, field.name, [quote(v, safe="") if isinstance(v, str) else v for v in value])
